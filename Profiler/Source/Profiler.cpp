...
namespace Profiler {
	...
	void serialize::CSVserialize2(GamingData gi) {
		...
		if (!firstTime) {
			//CSVPermanentInfo(gi, file);
			file << "DAY,TIME,CORES,CPU_LOAD,GPU_LOAD,MIN_GPU_LOAD,MAX_GPU_LOAD,USED_MEMORY,PEEK_MEMORY,RAM_LOAD,MAX_RAM_LOAD,GPU_TEMP,MIN_TEMP,MAX_TEMP\n";
			firstTime = 1;
		}
